  ...
  return (
    <div className="site-shell">
      ...
      <main id="top">
        ...
        <section id="connect" className="connect-section"><div className="wrap connect"><div><p className="eyebrow">We would love to hear from you</p><h2>Take the<br /><em>next step.</em></h2><p>Whether you are brand new to church or have been around for years, there is a place for you here.</p></div><form className="connect-form" onSubmit={async (event) => { event.preventDefault(); setContactStatus('Sending...'); const form = Object.fromEntries(new FormData(event.currentTarget)); try { const response = await fetch('/api/contact', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(form) }); if (!response.ok) throw new Error('The server could not accept this message.'); setContactStatus('Thanks. Our team will be in touch soon.'); event.currentTarget.reset() } catch (error) { setContactStatus(error instanceof TypeError ? 'The server is offline. Please start the API with npm run server.' : error.message) } }}><label>Name<input name="name" placeholder="Your name" required /></label><label>Email<input name="email" type="email" placeholder="you@example.com" required /></label><label>How can we help?<select name="subject" defaultValue=""><option value="" disabled>Choose one</option><option>Plan a visit</option><option>Join a group</option><option>Prayer request</option></select></label><textarea name="message" placeholder="Tell us a little more..." required></textarea><button className="button button-light" type="submit">Send a message <Icon name="send" size={16} /></button>{contactStatus && <small className="contact-status">{contactStatus}</small>}</form></div></section>
      </main>
