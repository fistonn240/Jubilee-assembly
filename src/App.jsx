...
function App() {
  ...
  function askQuestion(value = question) {
    ...
    setChat((items) => [...items, { from: 'user', text: prompt }, { from: 'ai', text: answer }])
    setQuestion('')
  }
  ...
  return (
    <div className="site-shell">
      ...
      <main id="top">
        ...
        <section className="service-strip"><div className="wrap service-inner"><div className="service-heading"><span className="live-dot"></span><span>Join us this week</span></div>{services.map((service) => <div className="service" key={service.day + service.time}><strong>{service.day}</strong><span>{service.time}</span><small>{service.label}</small></div>)}<a className="text-link" href="#gather">View gatherings <Icon name="arrow" size={15} /></a></div></section>
        ...
        <section id="events" className="events wrap"><div className="section-heading"><div><p className="eyebrow">Don't miss a thing</p><h2>Coming <em>up.</em></h2></div><a className="text-link" href="#events">See all events <Icon name="arrow" size={15} /></a></div><div className="event-list">{events.map((event) => <article className="event-row" key={event.title}><div className={`event-date ${event.tone}`}><strong>{event.date}</strong><span>{event.month}</span></div><div className="event-title"><h3>{event.title}</h3><p><Icon name="calendar" size={15} /> {event.meta}</p></div><a className="round-arrow" href="#connect" aria-label={`Learn more about ${event.title}`}><Icon name="arrow" size={18} /></a></article>)}</div></section>
        ...
        <section id="connect" className="connect-section"><div className="wrap connect"><div><p className="eyebrow">We would love to hear from you</p><h2>Take the<br /><em>next step.</em></h2><p>Whether you are brand new to church or have been around for years, there is a place for you here.</p></div><form className="connect-form" onSubmit={(event) => event.preventDefault()}><label>Name<input placeholder="Your name" /></label><label>Email<input type="email" placeholder="you@example.com" /></label><label>How can we help?<select defaultValue=""><option value="" disabled>Choose one</option><option>Plan a visit</option><option>Join a group</option><option>Prayer request</option></select></label><button className="button button-light" type="submit">Send a message <Icon name="send" size={16} /></button></form></div></section>
      </main>
